...
/* Global variables */
var calField; var otherDate; var calSpan; var calFormat; var calWknd = false;
...
function showCal(m,y,otherInput,f,dateSpan,wknd,format,evt,posType){
	...
	calFormat = format; 
	calWknd = wknd;
	otherDate = otherInput;
	if (calField.value != "" && calField.value != format) {
		var arr = calField.value.split("/");
		selectedM = parseFloat(arr[0])-1;
		selectedD = parseFloat(arr[1]);
		selectedY = parseFloat(arr[2]);		
	}
	...
	if (splitter != '') {		
		...
		if (otherInput == 'returnDate') {
			m2 = m; y2 = y; d2 = d; d = curr_dy; //m = curr_mn; y = curr_yr; 
			//Original line
//			calSpan = 6;
			//alert(m2 + " : " + m + " : " + y + " : " + y2 + " : " + d + " : " + curr_dy);
			//I added this line
			calSpan = 2;
			//if (m < m2 && y == y2) {
				//Original line
...
function placeDate(m,d,y){ 
	...
	document.getElementById(otherDate).value = getFormattedDate(m,d+1,y,calFormat);	
	hideCal();
}
